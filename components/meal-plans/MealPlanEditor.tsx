...
import { CheckCircle2, ChevronLeft, ChevronRight, LoaderCircle, Plus, Trash2, Utensils } from "lucide-react";
...
type Props = { weekStartDate: string; availableMealSlots: MealSlotSelection[]; plan: WeeklyMealPlan | null; items: PlannedMealItem[]; mealCategories: MealCategory[]; mealSlotTypes: MealSlotType[]; recipes: Pick<Recipe, "id" | "name" | "servings">[]; recipeMealCategoryIds: Record<string, string[]>; completedCheckoutSessionId: string | null };
...
export default function MealPlanEditor({ weekStartDate, availableMealSlots, plan, items, mealCategories, recipes, recipeMealCategoryIds, completedCheckoutSessionId }: Props) {
  ...
  const addableMealCategories = mealCategories.filter((category) => ["breakfast", "lunch", "dinner", "snacks", "desserts"].includes(category.slug));
  const manualDateBounds = getManualDateBounds(weekStartDate);
  const form = useForm<AddMealPlanItemInput>({ resolver: zodResolver(addMealPlanItemSchema), defaultValues: { meal_plan_id: plan?.id ?? "", meal_date: manualDateBounds.min, meal_category_id: addableMealCategories[0]?.id ?? "", recipe_id: "", servings: null, notes: "" } });
  ...
  if (plan?.status === "purchased") return <div className="space-y-6"><WeekHeader weekStartDate={weekStartDate} /><section className="rounded-3xl border border-emerald-400/20 bg-emerald-500/[0.08] p-6 backdrop-blur-xl sm:flex sm:items-center sm:justify-between sm:p-8"><div className="flex gap-4"><CheckCircle2 className="mt-0.5 h-7 w-7 shrink-0 text-emerald-300" aria-hidden="true" /><div><h2 className="text-xl font-semibold text-white">Groceries for this week have already been purchased.</h2><p className="mt-2 text-sm leading-relaxed text-emerald-100/75">This meal plan is now read-only.</p></div></div><div className="mt-5 flex flex-wrap gap-3 sm:mt-0">{completedCheckoutSessionId ? <Link href={`/dashboard/orders/${completedCheckoutSessionId}`} className="inline-flex h-11 items-center justify-center rounded-xl border border-emerald-300/40 bg-emerald-500/10 px-4 text-sm font-semibold text-emerald-100 transition hover:bg-emerald-500/20">View Order</Link> : <span className="inline-flex h-11 items-center justify-center rounded-xl border border-white/10 bg-white/5 px-4 text-sm font-semibold text-zinc-500">Order unavailable</span>}<Link href={`/dashboard/meal-plans?week=${getUpcomingWeekStart()}`} className="inline-flex h-11 items-center justify-center rounded-xl bg-emerald-400 px-4 text-sm font-semibold text-emerald-950 transition hover:bg-emerald-300">Plan Next Week</Link></div></section><PurchasedPlanMeals groupedItems={groupedItems} weekStartDate={weekStartDate} /></div>;
  ...
  async function addSlot(values: AddMealPlanItemInput) { const result = await addMealPlanItem(values); setMessage({ type: result.success ? "success" : "error", text: result.message }); if (result.success) { form.resetField("recipe_id", { defaultValue: "" }); form.resetField("servings", { defaultValue: null }); form.resetField("notes", { defaultValue: "" }); router.refresh(); } }
