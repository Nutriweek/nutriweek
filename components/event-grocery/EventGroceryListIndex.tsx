...
type EventGroceryList = { id: string; name: string; itemCount: number; updatedAt: string };
...
export default function EventGroceryListIndex({ lists }: { lists: EventGroceryList[] }) {
  ...
  function deleteList(id: string) {
    if (!window.confirm("Delete this event grocery list? This cannot be undone.")) return;
    startTransition(async () => {
      const result = await deleteEventGroceryList(id);
      setMessage({ type: result.success ? "success" : "error", text: result.message });
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-3xl border border-white/[0.08] bg-white/[0.04] p-5 sm:p-7" aria-labelledby="event-lists-heading"><div className="flex items-center gap-3"><div className="rounded-xl bg-cyan-500/10 p-2 text-cyan-200"><ClipboardList className="h-5 w-5" aria-hidden="true" /></div><div><h2 id="event-lists-heading" className="text-xl font-semibold text-white">Your Event Grocery Lists</h2><p className="mt-1 text-sm text-zinc-400">Keep every occasion organized in one place.</p></div></div>
      {lists.length === 0 ? <p className="mt-6 rounded-2xl border border-dashed border-white/10 px-4 py-10 text-center text-sm text-zinc-500">No event grocery lists yet. Create one to get started.</p> : <ul className="mt-5 divide-y divide-white/[0.08]">{lists.map((list) => <li key={list.id} className="flex flex-col gap-4 py-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold text-white">{list.name}</p><p className="mt-1 text-sm text-zinc-500">{list.itemCount} item{list.itemCount === 1 ? "" : "s"} · Updated {new Intl.DateTimeFormat("en-IN", { day: "numeric", month: "short", year: "numeric" }).format(new Date(list.updatedAt))}</p></div><div className="flex flex-wrap gap-2"><Link href={`/dashboard/event-grocery/${list.id}`} className="inline-flex items-center justify-center rounded-xl border border-emerald-400/30 bg-emerald-500/[0.08] px-4 py-2 text-sm font-semibold text-emerald-200 transition hover:bg-emerald-500/[0.14]">Open / Edit</Link><button type="button" onClick={() => deleteList(list.id)} disabled={isPending} className="inline-flex items-center gap-2 rounded-xl border border-rose-400/30 bg-rose-500/10 px-3 py-2 text-sm font-medium text-rose-200 transition hover:bg-rose-500/15 disabled:cursor-not-allowed disabled:opacity-60"><Trash2 className="h-4 w-4" aria-hidden="true" />Delete</button></div></li>)}</ul>}</section>
  </div>;
}
