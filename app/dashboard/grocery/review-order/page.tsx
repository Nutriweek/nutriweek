...
import PlaceOrderForm from "@/components/grocery/PlaceOrderForm";
import { getCheckoutSessionDetails } from "@/lib/grocery/checkoutSessions";
import { getCustomerAddresses } from "@/lib/local-store/customerAddresses";
...
export default async function ReviewOrderPage({ searchParams }: ReviewOrderPageProps) {
  ...
  const [checkoutSession, addresses] = await Promise.all([getCheckoutSessionDetails(session ?? ""), getCustomerAddresses()]);
