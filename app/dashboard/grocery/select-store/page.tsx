...
export default async function SelectStorePage({ searchParams }: SelectStorePageProps) {
  ...
  return <section className="mx-auto max-w-5xl space-y-6" aria-labelledby="select-store-heading">
    ...
    {providers.length > 0 ? <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{providers.map((provider) => { const selectable = provider.id === "local_store" && provider.status === "active"; return <form key={provider.id} action={selectCheckoutSessionProvider}><input type="hidden" name="session" value={session ?? ""} /><input type="hidden" name="provider" value={provider.id} /><button type="submit" disabled={!selectable} className={`group w-full rounded-3xl border border-white/[0.08] bg-white/[0.04] p-5 text-left transition ${selectable ? "hover:border-emerald-400/40 hover:bg-emerald-500/[0.08] hover:shadow-lg hover:shadow-emerald-950/20" : "cursor-not-allowed opacity-55"}`}><div className="flex items-center gap-4">{provider.icon ? <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white/10 text-2xl" aria-label={`${provider.name} logo`}>{provider.icon}</span> : <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-emerald-500/10 text-emerald-300"><Store className="h-6 w-6" aria-hidden="true" /></span>}<div><div className="flex flex-wrap items-center gap-2"><h2 className={`font-semibold ${selectable ? "text-white group-hover:text-emerald-100" : "text-zinc-400"}`}>{provider.name}</h2>{!selectable ? <span className="rounded-full border border-white/10 bg-white/[0.06] px-2 py-0.5 text-xs font-semibold uppercase tracking-wide text-zinc-400">Coming Soon</span> : null}</div>{providerMetadata[provider.id] ? <p className="mt-1 text-sm text-zinc-400">{providerMetadata[provider.id]}</p> : null}</div></div></button></form>; })}</div> : <div className="rounded-3xl border border-dashed border-white/15 bg-white/[0.03] px-6 py-12 text-center text-sm text-zinc-400">No shopping providers are available.</div>}
  </section>;
}
