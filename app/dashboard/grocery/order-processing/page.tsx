...
import DeliveryConfirmationForm from "@/components/grocery/DeliveryConfirmationForm";
...
function statusCopy(order: Awaited<ReturnType<typeof getCustomerLocalStoreOrderStatus>>) {
  ...
  if (order.fulfillmentStatus === "preparing") return { icon: Store, title: "Store is preparing your order", detail: "Your local store is preparing the order. The grocery amount is paid directly to the store on delivery." };
  if (order.fulfillmentStatus === "out_for_delivery") return { icon: Store, title: "Order is out for delivery", detail: "Your local store is delivering your order. Pay the grocery amount directly to the store on delivery." };
  if (order.fulfillmentStatus === "delivered") return { icon: Store, title: "Confirm delivery", detail: "Your store has marked the order delivered. Enter the delivery code after checking that everything was received correctly." };
