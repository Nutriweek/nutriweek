...
export function getNavigableWeekStart(value?: string) {
  const requestedWeekStart = getWeekStart(value);
  const upcomingWeekStart = getUpcomingWeekStart();
  return requestedWeekStart > upcomingWeekStart
    ? upcomingWeekStart
    : requestedWeekStart;
}

export const PLANNING_MEAL_CATEGORIES = [
  "breakfast",
  "lunch",
  "dinner",
] as const;
...
export type MealSlotSelection = {
  meal_date: string;
  meal_category_slug: PlanningMealCategory;
};

const mealCutoffs: Record<PlanningMealCategory, number> = {
  breakfast: 10 * 60,
  lunch: 14 * 60,
  dinner: 22 * 60,
};

export function getAvailableMealSlots(
  weekStartDate: string,
  now = new Date(),
): MealSlotSelection[] {
  const currentWeekStart = getWeekStart();
  const upcomingWeekStart = getUpcomingWeekStart();
  if (weekStartDate !== currentWeekStart && weekStartDate !== upcomingWeekStart)
    return [];
  ...
  if (
    ...
    currentMinutes >= mealCutoffs.dinner
  )
    return [];
  return Array.from({ length: 6 }, (_, offset) => {
    ...
    PLANNING_MEAL_CATEGORIES.flatMap((mealCategory) =>
      weekStartDate === upcomingWeekStart ||
      mealDate > today ||
      (mealDate === today && currentMinutes < mealCutoffs[mealCategory])
