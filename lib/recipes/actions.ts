...
async function hasDuplicateRecipeName(
  recipeName: string,
  userId: string,
  ...
  return [...(systemRecipes.data ?? []), ...(userRecipes.data ?? [])].some(
    (recipe) =>
      recipe.id !== excludeRecipeId &&
      normalizeRecipeName(recipe.name) === target,
  );
}
...
export async function createRecipe(
  ...
  if (!parsed.success)
    return {
      success: false,
      message: "Please correct the recipe details and try again.",
    };
  ...
  if (!user)
    return {
      success: false,
      message: "Please sign in before creating a recipe.",
    };

  const value = parsed.data;
  if (await hasDuplicateRecipeName(value.name, user.id))
    return {
      success: false,
      message:
        "A Nutriweek recipe or one of your recipes already uses this name.",
    };
  ...
  const { data: recipe, error: recipeError } = await supabase
    ...
    .insert({
      ...
      total_time_minutes:
        (value.prep_time_minutes ?? 0) + (value.cook_time_minutes ?? 0),
      ...
      nutrition_updated_at:
        value.calories_kcal === null ? null : new Date().toISOString(),
    })
    .select("id")
    .single();
  if (recipeError || !recipe)
    return { success: false, message: "We could not create this recipe." };
  ...
  const [
    ...
    supabase
      ...
      .insert(
        value.ingredients.map((ingredient, index) => ({
          ...
          base_unit_code: ingredient.base_unit_code || null,
        })),
      ),
    supabase
      .from("recipe_steps")
    ...
    supabase
      ...
      .insert(
        value.meal_category_ids.map((meal_category_id) => ({
          recipe_id: recipe.id,
          meal_category_id,
        })),
      ),
    value.tag_ids.length
      ? supabase
          ...
          .insert(
            value.tag_ids.map((tag_id) => ({ recipe_id: recipe.id, tag_id })),
          )
      : Promise.resolve({ error: null }),
    value.equipment_ids.length
      ? supabase
          .from("recipe_equipment_requirements")
          .insert(
            value.equipment_ids.map((equipment_id) => ({
              recipe_id: recipe.id,
              equipment_id,
            })),
          )
  ...
  if (
    ...
    return {
      success: false,
      message: "We could not save every recipe detail. Nothing was published.",
    };
  }
  ...
  return {
    success: true,
    message: "Your private recipe has been saved.",
    recipeId: recipe.id,
  };
}
...
export async function updateRecipe(
  ...
  if (!user)
    return {
      success: false,
      message: "Please sign in before editing a recipe.",
    };
  const value = parsed.data;
  if (await hasDuplicateRecipeName(value.name, user.id, recipeId))
    return {
      success: false,
      message:
        "A Nutriweek recipe or one of your recipes already uses this name.",
  ...
  if (recipeError)
    return { success: false, message: "We could not update this recipe." };
  ...
  if (removeIngredients.error || removeSteps.error || removeCategories.error)
    return {
      success: false,
      message: "We could not update every recipe detail.",
    };
  ...
  if (ingredientsResult.error || stepsResult.error || categoriesResult.error)
    return {
      success: false,
      message: "We could not save every recipe detail.",
    };
  revalidatePath("/dashboard/recipes");
...
export async function deleteRecipe(
  ...
  if (!user)
    return {
      success: false,
      message: "Please sign in before deleting a recipe.",
    };
  const { data: recipe, error: recipeError } = await supabase
    .from("recipes")
    ...
    .maybeSingle();
  if (recipeError || !recipe)
    return { success: false, message: "We could not find this recipe." };
  ...
  if (imagePath) {
    ...
    if (imageError)
      return {
        success: false,
        message:
          "We could not remove this recipe image. The recipe was not deleted.",
      };
  }
  const { error } = await supabase
    ...
    .eq("created_by", user.id);
  if (error)
    return { success: false, message: "We could not delete this recipe." };
  revalidatePath("/dashboard/recipes");
...
export async function uploadRecipeImage(
  ...
  if (
    ...
    return { success: false, message: "Use a JPG, JPEG, PNG, or WebP image." };
  if (image.size > 5 * 1024 * 1024)
    return { success: false, message: "Use an image smaller than 5 MB." };
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  ...
  if (error)
    return {
      success: false,
      message: `We could not upload that image: ${error.message}`,
    };
